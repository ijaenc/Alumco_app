...
import {
  ArrowLeft, Search, Download, Award, Calendar,
  User, Hash, Share2, Loader2,
} from "lucide-react";
...
export default function Certificates() {
  ...
  const handleShare = (cert: Certificate) => {
    navigator.clipboard?.writeText(`https://capacitaciones.ong/certificados/${cert.certificate_number}`).catch(() => {});
    toast.success("Enlace copiado al portapapeles", { description: "Puedes compartir tu certificado", duration: 3000 });
  };
  ...
  const thisYearCount = certificates.filter((c) => new Date(c.issued_at).getFullYear() === thisYear).length;
        ...
        <Card className="p-6 mb-6 bg-gradient-to-br from-blue-50 to-indigo-50 border-blue-200">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-gray-600">Certificados completados</p>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="bg-white/80 backdrop-blur-sm rounded-xl p-3">
              ...
              <p className="text-lg font-bold text-gray-900">{thisYearCount}</p>
            </div>
          </div>
        </Card>
        ...
        {filteredCertificates.length > 0 ? (
          <div className="space-y-4">
            {filteredCertificates.map((cert) => (
              <Card key={cert.id} className={`overflow-hidden transition-all ${selectedCert === cert.id ? "ring-2 ring-primary shadow-lg" : "hover:shadow-md"}`}>
                {/* Certificate Header */}
                <div className="bg-gradient-to-r from-blue-500 to-indigo-600 p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="font-semibold text-white mb-1 leading-tight">{cert.course_title}</h3>
                      <div className="flex items-center gap-2 text-xs text-white/90">
                        <Hash className="w-3 h-3" />
                        <span>{cert.certificate_number}</span>
                      </div>
                    </div>
                    <div className={`px-3 py-1.5 rounded-full font-semibold text-sm ${getScoreColor(cert.score)}`}>
                      {cert.score}%
                    </div>
                  </div>
                </div>
                ...
                <div className="p-4 bg-white">
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-sm font-medium text-gray-900 truncate">{cert.instructor_name}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button variant="outline" onClick={() => handleShare(cert)} className="h-10 border-gray-300">
                  ...
                  {selectedCert === cert.id && (
                    <div className="mt-4 pt-4 border-t border-gray-100">
                      <h4 className="text-sm font-semibold text-gray-900 mb-3">Detalles del certificado</h4>
                      <div className="space-y-2 text-sm">
                        <div className="flex justify-between">
                          <span className="text-gray-600">Estado:</span>
                          <span className="font-medium text-green-600 capitalize">{cert.status === "active" ? "Activo" : cert.status}</span>
                        </div>
                        <div className="flex justify-between">
                        ...
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium text-gray-900 text-xs">{cert.certificate_number}</span>
                        </div>
                      </div>
                    </div>
                  )}
          ...
          <div className="flex flex-col items-center justify-center py-16 px-4">
            ...
            <p className="text-sm text-gray-600 text-center mb-6">
              {searchQuery ? "Intenta con otra búsqueda" : "Completa una capacitación para obtener tu primer certificado"}
            </p>
            {!searchQuery && (
              <Button onClick={() => navigate("/dashboard")} className="bg-primary hover:bg-blue-600">
                Ver Capacitaciones
              </Button>
            )}
          </div>
