...
export async function deleteCourse(req: Request, res: Response): Promise<void> {
  ...
  // Borrar en orden estricto respetando foreign keys
  // 1. Progreso de módulos
  await db("module_progress").whereIn("module_id",
    db("modules").where({ course_id: id }).select("id")
  ).delete();
  // 2. Certificados (referencian attempts)
  await db("certificates").where({ course_id: id }).delete();
  // 3. Intentos (referencian evaluaciones)
...
export async function addModule(req: Request, res: Response): Promise<void> {
  ...
  const schema = z.object({ title: z.string().min(1), description: z.string().optional(), type: z.enum(["video","document","text","image"]).default("video"), content_url: z.string().optional().nullable(), content: z.string().optional(), duration: z.string().optional().nullable(), order_index: z.number().int().min(0).default(0) });
...
export async function updateModule(req: Request, res: Response): Promise<void> {
  ...
  const updateModSchema = z.object({
    ...
    type: z.enum(["video","document","text","image"]).optional(),
    content_url: z.string().optional().nullable(),
    content: z.string().optional().nullable(),
    duration: z.string().optional().nullable(),
    order_index: z.number().int().min(0).optional(),
  });
  const parsedMod = updateModSchema.safeParse(req.body);
  if (!parsedMod.success) { res.status(400).json({ error: "Datos inválidos" }); return; }
  await db("modules").where({ id: moduleId }).update(parsedMod.data);
